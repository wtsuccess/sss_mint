...
export default function MintingModal({
  ...
  const increaseAmount = () => {
    if (amount < maxNum - mintedCount.toNumber() - 10) {
      setAmount(amount + 1);
    }
  };
    ...
    >
      <div className="fixed top-0 left-0 w-screen h-screen text-white z-20">
        ...
        >
          <label
            htmlFor=""
            className="absolute top-[6px] right-[5px] py-[0.5px] px-[5px] text-[12px] text-[#a91313] bg-[#dfdda1] rounded-[7px]"
          >
            {getSalePhase(saleMode)}
          </label>

          <div
            className="flex flex-1 w-[100%] flex-col items-center text-center"
            onKeyDown={handleKeyEvent}
          >
            <p className="scale-x-[1.7] text-[38px] italic font-[800] leading-[44.7px] font-['Sequel 100 Black']">
              {mintedCount.toNumber()}/{totalCount.toNumber() - 10}
            </p>
            <p className="text-[28px] font-['Abel']">SEALS</p>
            <p className="text-[28px] font-['Abel']">
              1 Seal = {parseFloat(ethers.utils.formatEther(price)).toFixed(2)}{" "}
              ETH
            </p>
            <p className="mt-[37px] text-[32px] font-['Abel']">How many?</p>
            <div className="flex justify-center mt-[14.5px]">
              ...
              <div className="self-center ml-[18px]">
                <svg
                  ...
                  width="25"
                  height="17"
                  className="cursor-pointer h-[15px] md:h-[20px] transition ease-in-out delay-100 hover:scale-125 duration-200 bg-white"
                  onClick={() => {
                    increaseAmount();
                  }}
                >
                ...
                <svg
                  ...
                  className="rotate-180 cursor-pointer h-[20px] transition ease-in-out delay-100 hover:scale-125 duration-200 bg-white"
                >
                  <defs>
                    <image
                      width="25"
            ...
            </div>
            {
              <button
                ...
                className="mt-auto group flex justify-center items-center relative md:w-full h-[80px] w-[100%] bg-orange-500 text-white rounded-2xl lg:text-[38px] md:text-[32px] text-[28px] font-bold font-['Abel'] cursor-pointer overflow-hidden transition duration-200 ease-in-out hover:bg-orange-400"
              >
                MINT
                {/* NFT Art price */}
                <span className="ml-5">
